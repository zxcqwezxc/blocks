...
// Функция для получения цвета по значению блока
...
const Tile: React.FC<TileProps> = ({ value, rowIndex, isMerged }) => {
  const translateY = useSharedValue(100 * (7 - rowIndex));
  ...
  useEffect(() => {
    if (value !== null) {
      if (isMerged) {
        ...
        translateY.value = 100 * (7 - rowIndex);
